...
try {
  ...
  const [root, deepLink, configuration, search] = await Promise.all([
    fetch(`${origin}/`),
  ...
  if (![root, deepLink, configuration, search].every((response) => response.ok)) {
    throw new Error(`Unexpected status: ${[root, deepLink, configuration, search].map((item) => item.status).join(', ')}`)
  }
  ...
  console.log(`Smoke passed: root=${root.status}, deep=${deepLink.status}, config=${configuration.status}, search=${search.status}`)
} catch (error) {
